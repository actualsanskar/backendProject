...
router.route('/new-access-token').post(refreshingAccessToken);
...
router.route('/update-avatar').patch(verify_JWT, upload.single('newAvatarFilePath'), updateUserAvatar)
router.route('/update-cover-image').patch(verify_JWT, upload.single("newCoverImageFilePath"), updateUserCoverImage),
router.route('/channel/:username').get(verify_JWT, getUserChannelProfile)
router.route('/watch-history').get(verify_JWT, watchHistory)
